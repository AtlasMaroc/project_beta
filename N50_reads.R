...
#specifying your working directory using setwd

library(here)

file_dir <- here()
file_path <- here(file_path, "reads.length.csv")
...
read_length_df <- read.csv(file_path)
...
read_length_df$platform <- factor(read_length_df$platform,level = c("ONT"))
